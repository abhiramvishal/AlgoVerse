...
function FieldLabel({ text }: { text: string }) {
  return (
    <span className="shrink-0 w-14 text-[10px] font-bold font-mono uppercase tracking-wider text-zinc-500">
      {text}
    </span>
  );
}
...
interface InputPanelProps {
  ...
  /** Compact mode: no card wrapper/header — just flat fields + Apply button inline */
  compact?: boolean;
}

// ─── Component ────────────────────────────────────────────────────────────────

export function InputPanel({ defaultInput, input, onApplyInput, compact = false }: InputPanelProps) {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(input)) { setArrayRaw((input as number[]).join(", ")); return; }
  ...
  const applyBtn = (
    <Button
      size="sm"
      onClick={handleApply}
      className="h-8 shrink-0 px-3 rounded-xl bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-600/30 hover:text-indigo-200 text-xs font-semibold active:scale-95 transition-all"
    >
      Apply
    </Button>
  );
    ...
    return (
      <div className="flex items-center gap-2 rounded-xl border border-white/[0.04] bg-zinc-950/30 px-4 py-2.5">
        ...
        <span className="text-[11px] text-zinc-600 font-mono">
          Fixed internal dataset — no custom input
        </span>
      </div>
    );
  }

  // ── Compact mode: flat rows, no card ─────────────────────────────────────────
  if (compact) {
    return (
      <div className="space-y-1.5">
        {kind === "number-array" && (
          <Row>
            ...
            <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. 8, 4, 2, 9" className={fieldCls} />
            {applyBtn}
          </Row>
        )}
        {kind === "array-target" && (
          <>
            <Row>
              ...
              <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 2, 5, 8, 12" className={fieldCls} />
            </Row>
            <Row>
              <FieldLabel text="Target" />
              <Input value={targetRaw} onChange={(e) => setTargetRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 8" className={`${fieldCls} max-w-[100px]`} />
              {applyBtn}
            </Row>
          </>
        )}
        {kind === "array-k" && (
          <>
            <Row>
              ...
              <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 2, 1, 5, 3" className={fieldCls} />
            </Row>
            <Row>
              <FieldLabel text="k" />
              <Input value={kRaw} onChange={(e) => setKRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 3" className={`${fieldCls} max-w-[80px]`} />
              {applyBtn}
            </Row>
          </>
        )}
        {kind === "single-number" && (
          <Row>
            <FieldLabel text="n" />
            <Input value={numRaw} onChange={(e) => setNumRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. 8" className={`${fieldCls} max-w-[100px]`} />
            {applyBtn}
          </Row>
        ...
        {kind === "single-string" && (
          <Row>
            ...
            <Input value={strRaw} onChange={(e) => setStrRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. 110" className={fieldCls} />
            {applyBtn}
          </Row>
        )}
        {kind === "str-wrap" && (
          <Row>
            ...
            <Input value={strWrapRaw} onChange={(e) => setStrWrapRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. banana" className={fieldCls} />
            {applyBtn}
          </Row>
        )}
        {kind === "two-strings" && (
          <>
            <Row>
              <FieldLabel text="Str A" />
              <Input value={s1Raw} onChange={(e) => setS1Raw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. kitten" className={fieldCls} />
            </Row>
            <Row>
              <FieldLabel text="Str B" />
              <Input value={s2Raw} onChange={(e) => setS2Raw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. sitting" className={fieldCls} />
              {applyBtn}
            </Row>
          </>
        )}
        {kind === "json" && (
          <Row>
            <textarea value={jsonRaw} onChange={(e) => setJsonRaw(e.target.value)} rows={2}
              className="flex-1 rounded-xl bg-zinc-950/60 border border-white/[0.06] text-[11px] text-zinc-300 font-mono p-2 leading-relaxed resize-none focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
              spellCheck={false} />
            {applyBtn}
          </Row>
        )}
        {error && <p className="text-[10px] text-rose-400 font-mono pl-16">{error}</p>}
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {kind === "number-array" && (
          <Row>
            ...
            <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. 8, 4, 2, 9, 5, 7" className={fieldCls} />
          </Row>
        )}
        {kind === "array-target" && (
          <>
            <Row>
              ...
              <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 2, 5, 8, 12, 23, 38" className={fieldCls} />
            </Row>
            <Row>
              <FieldLabel text="Target" />
              <Input value={targetRaw} onChange={(e) => setTargetRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 23" className={`${fieldCls} max-w-[130px]`} />
            </Row>
          </>
        )}
        {kind === "array-k" && (
          <>
            <Row>
              ...
              <Input value={arrayRaw} onChange={(e) => setArrayRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 2, 1, 5, 1, 3, 2" className={fieldCls} />
            </Row>
            <Row>
              <FieldLabel text="k" />
              <Input value={kRaw} onChange={(e) => setKRaw(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleApply()}
                placeholder="e.g. 3" className={`${fieldCls} max-w-[90px]`} />
            </Row>
          </>
        )}
        {kind === "single-number" && (
          <Row>
            ...
            <Input value={numRaw} onChange={(e) => setNumRaw(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleApply()}
              placeholder="e.g. 8" className={`${fieldCls} max-w-[130px]`} />
          </Row>
        )}
        {kind === "single-string" && (
          <Row>
        ...
        {kind === "json" && (
          <textarea value={jsonRaw} onChange={(e) => setJsonRaw(e.target.value)} rows={4}
            className="w-full rounded-xl bg-zinc-950/60 border border-white/[0.06] text-[11px] text-zinc-300 font-mono p-2.5 leading-relaxed resize-none focus:outline-none focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500/40"
            spellCheck={false} />
        )}
      </div>
